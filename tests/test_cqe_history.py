"""
Unit tests for CQE history functionality
"""
...
class TestCQEHistoryFileParsing(unittest.TestCase):
    ...
    def test_parse_valid_history(self):
        ...
        content = {
            ...
            "queries": [
                ...
                {
                    "query": "title ~ 'Dokumentation'",
                    "last_used": "2026-01-26T14:25:00",
                },
            ],
        }
        ...
        self.assertEqual(result[0]["query"], "type = page AND space = DEV")
        self.assertEqual(result[1]["query"], "title ~ 'Dokumentation'")
...
class TestCQEHistoryDuplicateHandling(unittest.TestCase):
    ...
    def test_add_new_query(self):
        ...
        queries = []
        result = self._add_to_history(queries, "type = page AND space = DEV")
    ...
    def test_add_duplicate_moves_to_top(self):
        ...
        queries = [
            {"query": "title ~ 'API'", "last_used": "2026-01-26T14:30:00"},
            {
                "query": "type = page AND space = DEV",
                "last_used": "2026-01-26T14:25:00",
            },
        ...
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]["query"], "type = page AND space = DEV")
        self.assertEqual(result[1]["query"], "title ~ 'API'")
    ...
    def test_query_whitespace_stripped(self):
        ...
        queries = []
        result = self._add_to_history(queries, "  type = page  ")

        self.assertEqual(result[0]["query"], "type = page")


class TestCQEHistoryMultipleAdds(unittest.TestCase):
    """Test adding multiple queries to history in sequence"""

    def setUp(self):
        """Create temporary directory for test files"""
        self.temp_dir = tempfile.mkdtemp()
        self.history_file = os.path.join(self.temp_dir, "test_cqe_history.json")
...
class TestCQEHistoryFileOperations(unittest.TestCase):
    ...
    def test_save_and_load(self):
        ...
        queries = [
            {"query": "type = page", "last_used": "2026-01-26T14:30:00"},
        ]
        self._save_history(queries, self.history_file)
        result = self._load_history(self.history_file)
    ...
    def test_unicode_queries(self):
        ...
        queries = [
            {"query": "title ~ 'Übersicht'", "last_used": "2026-01-26T14:30:00"},
            {"query": "title ~ '日本語'", "last_used": "2026-01-26T14:25:00"},
        ]
        self._save_history(queries, self.history_file)
        result = self._load_history(self.history_file)

        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]["query"], "title ~ 'Übersicht'")
        self.assertEqual(result[1]["query"], "title ~ '日本語'")


class TestCQEHistoryClear(unittest.TestCase):
...
class TestCQEHistoryInputDetection(unittest.TestCase):
...
class TestCQEVirtualQueryModeConditions(unittest.TestCase):
    """Test Virtual Query Mode condition checking logic

    Virtual Query Mode allows Tab on history entries to execute
    the CQL query directly and show results in Keypirinha.
    """
    ...
    def _check_virtual_query_mode_conditions(
        ...
        Args:
            current_mode: Current plugin mode (MODE_CQL or MODE_FILTER)
    ...
    def test_vqm_triggers_with_valid_history_entry(self):
        ...
        items_chain = [
            {"category": self.ITEMCAT_QUERY, "target": "cqe"},
            {
                "category": self.ITEMCAT_HISTORY,
                "target": "history_entry_0",
                "data_bag": "type = page AND space = DEV",
            },
        ]
        result = self._check_virtual_query_mode_conditions(
            self.MODE_CQL, items_chain, self.ITEMCAT_HISTORY, "history_entry_"
        )
    ...
    def test_vqm_not_triggered_with_wrong_category(self):
        ...
        items_chain = [
            {"category": self.ITEMCAT_QUERY, "target": "cqe"},
            {"category": self.ITEMCAT_QUERY, "target": "execute_cql"},
        ]
        result = self._check_virtual_query_mode_conditions(
            self.MODE_CQL, items_chain, self.ITEMCAT_HISTORY, "history_entry_"
        )
    ...
    def test_vqm_empty_chain(self):
        ...
        result = self._check_virtual_query_mode_conditions(
            self.MODE_CQL, [], self.ITEMCAT_HISTORY, "history_entry_"
        )
        self.assertFalse(result)


class TestCQEHistoryItemCreationParams(unittest.TestCase):
